...
<meta name="description" content="Portfolio of DevOps Internship Tasks: Ansible, Terraform, Docker, Jenkins, EC2 Deployments">
<style>
:root {
  --bg: #0f1724;
  --card: #0b1220;
...
<header>
  ...
  <div>
    ...
    <p class="lead">6 internship tasks completed — Terraform, Docker, Ansible, Jenkins, EC2 deployments. Click task cards to view docs or screenshots.</p>
  </div>
...
<div class="card">
  ...
  <p class="muted">This portfolio contains full documentation and screenshots for all 6 internship tasks. Each task includes step-by-step commands, configuration files, and verification steps to demonstrate DevOps skills.</p>
  ...
  <details>
    ...
    <div class="note">Place this file as <code class="kbd">index.html</code> in your repo root. Put screenshots in <code class="kbd">/screenshots/</code>. Enable GitHub Pages from repo settings to publish this page.</div>
  </details>
</div>

<div class="card">
  <h3>Quick Links</h3>
  <p class="muted">Open original docs stored in this repo or jump to the screenshot gallery.</p>
  <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
...
function createThumb(src, alt){
  ...
  img.onerror=()=>{img.src='data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="600" height="400"><rect width="100%" height="100%" fill="%23071C2A"/><text x="50%" y="50%" fill="%23fff" font-size="18" text-anchor="middle">Image not found</text></svg>'}
...
function renderGallery(){
  ...
  Object.keys(imagesMap).forEach(taskKey=>{
    imagesMap[taskKey].forEach(fname=>{
      const path='./screenshots/'+fname;
      wrap.appendChild(createThumb(path,fname));
    })
  });
  document.getElementById('galleryWrapper').innerHTML=''; document.getElementById('galleryWrapper').appendChild(wrap);
}
